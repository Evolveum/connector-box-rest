...
public class Main {
	...
	public static void main(String[] args) throws IOException, URISyntaxException {
		...
		System.out
				.println("-------------------------------------------------------------------------------------------");
		System.out.println("You have to create and register App in Box API.");
		...
		URI uri = new URIBuilder().setScheme("https").setHost("account.box.com").setPath("/api/oauth2/authorize?")
				.setParameter("response_type", "code").setParameter("client_id", clientId)
				.setParameter("redirect_uri", redirectUri).addParameter("state", "48d2LMuz8").build();
		...
		System.out
				.println("-------------------------------------------------------------------------------------------");
		System.out.println(
				"If page do not open in your web browser please copy and open the following link in your web browser: ");
		System.out.println("");
		System.out.println(httpget.getURI());
		System.out
				.println("-------------------------------------------------------------------------------------------");
		System.out.println(
				"Please copy the Authorization Code that you get in URL after clicking on Grant access to Box");
		System.out
				.println("-------------------------------------------------------------------------------------------");
		System.out
				.println("-------------!!!Authorization Code is valid only for 30sec. Please by hurry!!!-------------");
		...
		if (!authCode.isEmpty()) {
			...
			sb.append("grant_type").append('=').append(GRANT_TYPE).append('&').append("code").append('=')
					.append(authCode).append('&').append("client_id").append('=').append(clientId).append('&')
